...
/**
 * Shift bits right in the bitarray.
 *
 * @param bits : structure storing the bits under 64-bit integers.
 * @param n: the length of the shift (at most 8).
 * @post bits.size has been decreased by n (unless it reached 0).
 *       All the bits in the bitarray have been shifted to the right by n bits
 *       (ie. the n-th least significant bit is now the least significant, and so on)
 */
void right_shift_bit(bitarray256_t *bits, int n);

/**
 * Add a byte at the specified position in the bitarray
 *
 * @param bits: the bitarray
 * @param pos: position at which the bits start being copied
 * @param byte: the bits to be copied
 */
void add_byte(bitarray256_t *bits, int pos, int byte);

/**
 * @returns the value of the first 8 bits of the bitarray and shift right by 8 bits.
 */
int pop_byte(bitarray256_t *bits);
#endif
